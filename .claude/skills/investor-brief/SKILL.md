...
## Pexels Integration (MANDATORY — run BEFORE generating HTML)

**You MUST run the `pexels-images` skill BEFORE writing any HTML.** This is a hard prerequisite, not optional.

### Step-by-step process

1. **Fetch all photos first.** For each slide marked "YES — photo eligible" in the table above, query the Pexels API using the suggested search queries below. Use `curl` via Bash with the `PEXELS_API_KEY` environment variable:
   ```bash
   ...
   ```
   Parse JSON with `python` (not `python3` on Windows): extract `photos[0].src.large2x` and `photos[0].photographer`.

2. **Store results as variables.** Before writing the HTML file, collect all photo URLs and photographer names. You need 8 photos total (slides 1, 2, 3, 6, 7, 11, 14, 15).

3. **Embed directly in HTML.** Each photo-eligible slide uses the fetched URL as an inline `background-image` CSS property with a dark gradient veil overlay:
   ```css
   background: linear-gradient(135deg, rgba(8,10,24,.88) 0%, rgba(8,10,24,.72) 50%, rgba(8,10,24,.6) 100%),
               url('PEXELS_URL_HERE');
   background-size: cover;
   background-position: center;
   ```

4. **Add photo credit** on every photo slide: `<span>` positioned bottom-right, 9px, 18% opacity.

5. **Fallback:** If the API call fails, use `background: #0D0F1E;` — do not block generation.
...
### PDF version (generated FROM the HTML deck — not a separate document)
6. The PDF MUST be generated by converting the HTML slide deck to PDF using puppeteer's `page.pdf()`.
   Do NOT create a separate prose document or a different HTML file for the PDF.
   The PDF must look identical to the HTML deck: same slides, same photos, same layout.
7. Use puppeteer with these settings:
   ```javascript
   // Load the HTML file, wait for fonts + images
   await page.goto('file:///' + absolutePath, { waitUntil: 'networkidle0', timeout: 30000 });
   // Set viewport to 1920x1080 to match slide aspect ratio
   await page.setViewport({ width: 1920, height: 1080 });
   // Inject print CSS that makes each .slide a separate page
   await page.addStyleTag({ content: `
     html, body { height: auto; overflow: visible; }
     .deck { position: static; height: auto; }
     .slide { position: static; opacity: 1; pointer-events: auto; page-break-after: always;
   ...
   ```
   This produces one slide per PDF page in 16:9 landscape format with full background images.
